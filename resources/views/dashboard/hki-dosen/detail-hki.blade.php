...
<script>
    ...
    function ajukanHki(id) {
        ...
        $.ajax({
            ...
            success: function(response) {
                Swal.fire({
                    title: "Berhasil",
                    text: "Hki telah diajukan mohon menunggu notifikasi dari admin!",
                    icon: "success",
                    confirmButtonText: "Oke",
                }).then(() => {
                    window.location.reload();
    ...
    function uploadFileHki(hki_id) {
        let namaFile = $('#namaFile').val();
        let fileInput = $('#fileHki')[0];
        ...
        formSumberDaya.append('hki_id', hki_id);
        formSumberDaya.append('file', file);

        $.ajax({
            type: "POST",
            url: "{{ route('upload.file.hki') }}",
            data: formSumberDaya,
            contentType: false,
            processData: false,
            success: function(response) {
                Swal.fire({
                    title: "Berhasil",
                    text: "Bukti hki berhasil diunggah!",
                    icon: "success",
                    confirmButtonText: "Oke",
                }).then(() => {
                    window.location.reload();
    ...
    function hapusFileHki(file_id) {
        $.ajax({
            type: "GET",
            url: `/delete-file-hki/${file_id}`,
            data: {
                _token: "{{ csrf_token() }}"
            },
            success: function(response) {
                Swal.fire({
                    title: "Berhasil",
                    text: "Bukti hki berhasil dihapus!",
                    icon: "success",
                    confirmButtonText: "Oke",
                }).then((result) => {
...
<body class="bg-gray-100" x-data="{ sidebarOpen: false }">
    ...
    <main :class="sidebarOpen ? 'sm:ml-64' : 'sm:ml-16'"
        ...
        <x-navbar title="Detail Hki" description="Pantau dan kelola publikasi dari {{ $hki->nama_dosen }}" />
        ...
        <div class="flex flex-col lg:flex-row gap-4 w-full h-[700px]">
                            ...
                            <div class="w-full md:w-1/2 space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="jenis" id="jenis"
                                        ...
                                        placeholder="Masukkan jenis ciptaan" value="{{ $hki->jenis }}" required
                                        disabled />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="tahun" id="tahun"
                                        ...
                                        value="{{ $hki->tahun }}" required disabled />
                                </div>
                            </div>
                            ...
                            <div class="w-full md:w-1/2 space-y-4">
                                ...
                                <div>
                                    ...
                                    <textarea name="judul" id="judul"
                                        ...
                                        placeholder="Masukkan judul Ciptaan" required disabled>{{ $hki->judul }}</textarea>
                                </div>
                            </div>
                        </div>
                    </form>
                    ...
                    <div class="mt-4 space-y-5">
                        ...
                        @if ($fileHki->isEmpty())
                            <div class="text-center text-gray-500 font-medium p-4">
                                Anda belum mengunggah bukti hki.
                            </div>
                        @else
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                <!-- Card 1 -->
                                @foreach ($fileHki as $file)
                                    @php
                                        $tipe = '';
                                        if ($file->tipe == 'pdf') {
                                    ...
                                    <div
                                        ...
                                        <div class="flex gap-4">
                                            {{-- Tombol Hapus --}}
                                            <button type="button" onclick="hapusFileHki('{{ $file->id }}')"
                                            ...
                                            {{-- Tombol Download --}}
                                            <a href="{{ route('download.file.hki', $file->id) }}"
                                                class="relative group flex items-center justify-center w-10 h-10">
                        ...
                        @if (Auth::user()->role == 'dosen')
                            <div class="flex justify-end">
                                <div class="flex items-center justify-between gap-4 mt-4 mb-4">
                                    <button type="button" onclick="ubahHki('{{ $hki->id }}')"
                                    ...
                                    @if ($hki->status != 'Diajukan')
                                        <button type="button" onclick="ajukanHki('{{ $hki->id }}')"
                        ...
                        @if (Auth::user()->role == 'admin' && $hki->status != 'Draft')
                            <div class="flex justify-start">
                                <div class="flex items-center justify-between gap-4 mt-4 mb-4">
                                    <button type="button" onclick="setujuiHki('{{ $hki->id }}')"
                                        class="bg-[#48A6A7] border-2 border-[#006A71] hover:bg-white text-white hover:text-[#006A71] font-medium py-2 px-4 rounded-lg transition duration-300">
                                        Setujui Hki
                                    </button>
                                    <button type="button" onclick="tolakHki('{{ $hki->id }}')"
            ...
            <div class="w-full lg:w-[30%] flex flex-col gap-4 h-full">
                <div class="bg-white w-full p-4 shadow rounded overflow-y-auto h-[50%]">
                        ...
                        <div class="relative group">
                            ...
                            <div id="dropdownKomentar"
                                ...
                                <form action="#" method="POST" class="space-y-3">
                                    ...
                                    <button type="button" onclick="addKomentar('{{ $hki->id }}')"
                    ...
                    <div class="space-y-2">
                        @if ($komentarHki->isEmpty())
                            <div class="text-center text-gray-500 font-medium p-4">
                                Belum ada komentar.
                            </div>
                        @else
                            @foreach ($komentarHki as $data)
                        ...
                        <script>
                            function deleteKomentar(komentar_id) {
                                $.ajax({
                                    url: `/delete-komentar-hki/${komentar_id}`,
                                    type: 'GET',
                                    data: {
                                        _token: '{{ csrf_token() }}'
                                    },
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="relative group">
                            ...
                            <div id="dropdownKolaborasi"
                                ...
                                @foreach ($dosen as $data)
                                    <div class="flex items-center gap-4 w-full p-4 bg-gray-100 rounded-lg shadow-lg">
                                        ...
                                        <div class="flex justify-between w-full items-center">
                                            ...
                                            @if (Auth::user()->role == 'dosen')
                                                <button type="button"
                                                    onclick="addKolaborasi('{{ $data->id }}', '{{ $hki->id }}')"
                                ...
                                <script>
                                    function addKolaborasi(dosen_id, hki_id) {

                                        $.ajax({
                                            url: "{{ route('add.kolaborasi.hki') }}",
                                            type: 'POST',
                                            data: {
                                                dosen_id: dosen_id,
                                                hki_id: hki_id,
                                    ...
                                    function deleteKolaborasi(kolaborasi_id) {
                                        $.ajax({
                                            url: `/delete-kolaborasi-hki/${kolaborasi_id}`,
                                            type: 'GET',
                                            data: {
                                                _token: '{{ csrf_token() }}'
                                            },
        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-lg">
                        ...
                        <div class="mb-4">
                            ...
                            <div id="drop-area"
                                ...
                                <p class="text-sm text-gray-700 mt-4">Tarik dan letakkan gambar atau file di sini atau
                                    klik untuk unggah</p>
                                <input type="file" id="fileHki" multiple class="hidden"
                                    onchange="handleFiles(this.files)">
                                <button type="button" onclick="document.getElementById('fileHki').click()"
                                    class="mt-4 mb-6 text-white bg-[#48A6A7] hover:bg-[#006A71] font-medium rounded px-4 py-1 text-sm">
                                    Pilih File
                                </button>
                            </div>
                            <p id="helpFileHki" class="help is-hidden text-xs mt-2 mb-4 text-red-400"></p>
                ...
                <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b gap-4">
                    <button type="button" onclick="uploadFileHki('{{ $hki->id }}')"
