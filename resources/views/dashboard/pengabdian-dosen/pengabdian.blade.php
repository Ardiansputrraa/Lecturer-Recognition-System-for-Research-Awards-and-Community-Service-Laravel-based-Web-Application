...
<script>
    $(document).ready(function() {
        $('#search').on('keyup', function() {
            let query = $(this).val();
            if (query != "") {
                searchDataPengabdian(query);
            } else {
                window.location.reload();
            }
        });

        $('#searchTahunPengabdian').on('change', function() {
            let tahun = $(this).val();
            let query = $('#searchTahunPengabdian').val();
    ...
    function searchDataPengabdian(query) {
        $.ajax({
            url: "{{ route('search.pengabdian') }}",
            type: "GET",
            data: {
                _token: "{{ csrf_token() }}",
                keyword: query,
            ...
            success: function(data) {
                console.log(data);
                let tabelPengabdian = $("#tabelPengabdian");
                tabelPengabdian.empty();
                            ...
                            `<tr class="bg-white border-b">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-start gap-3">
                                        <!-- Button View -->
                                        <a href="/detail-pengabdian/${data[i]["id"]}" class="relative group flex items-center justify-center w-10 h-10">
                                        ...
                                        <!-- Button Delete -->
                                        <button class="relative group flex items-center justify-center w-10 h-10" type="button" onclick="deleteDataPengabdian('${data[i]["id"]}')">
    ...
    function addPengabdian() {
        ...
        if (jabatan === "") {
            $("#helpJabatan")
                .text("Silahkan masukan jabatan dalam pengabdian!")
                .removeClass("is-safe")
                .addClass("is-danger");
            $("#jabatan").focus();
            return;
        ...
        if (tahun === "") {
            $("#helpTahun")
                .text("Silahkan masukan tahun pengabdian!")
                .removeClass("is-safe")
                .addClass("is-danger");
            $("#tahun").focus();
            return;
        ...
        if (dana === "") {
            $("#helpDana")
                .text("Silahkan masukan dana pengabdian!")
                .removeClass("is-safe")
                .addClass("is-danger");
            $("#dana").focus();
            return;
        ...
        if (sumber === "") {
            $("#helpSumber")
                .text("Silahkan masukan sumber dana pengabdian!")
                .removeClass("is-safe")
                .addClass("is-danger");
            $("#sumber").focus();
            return;
        ...
        $.ajax({
            ...
            success: function(response) {
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil',
                    text: 'Data pengabdian berhasil ditambahkan!',
                }).then(() => {
                    location.reload();
                });
            },
...
<body class="bg-gray-100" x-data="{ sidebarOpen: false }">
    ...
    <main :class="sidebarOpen ? 'sm:ml-64' : 'sm:ml-16'"
        ...
        <x-navbar title="Pengabdian Dosen" description="Pantau dan kelola data pengabdian secara efisien" />

        <!-- Tabel Data Pengabdian -->
        <div class="bg-white rounded-2xl shadow-lg p-6 mt-8">

            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 mt-4">
                <!-- Tombol Tambah Pengabdian -->
                @if (Auth::user()->role == 'dosen')
                    <button data-modal-target="tambahPengabdianModal" data-modal-toggle="tambahPengabdianModal"
                        type="button"
                ...
                <div class="flex flex-row  items-stretch sm:items-center gap-4 w-full md:w-auto">
                    ...
                    <div class="relative w-full sm:w-64">
                        <input type="text" placeholder="Cari Pengabdian Dosen" id="search"
                    ...
                    <!-- Tombol Ekspor CSV -->
                    <a href="{{ route('download.pengabdian') }}"
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-12">
                ...
                <div class="w-full max-w-xs">
                    <select id="searchTahunPengabdian" name="searchTahunPengabdian"
                        ...
                        @foreach ($pengabdian as $data)
                            <tr class="bg-white border-b">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-start gap-3">
                                        <!-- Button View -->
                                        <a href="{{ url('/detail-pengabdian/' . $data->id) }}"
                                        ...
                                        <!-- Button Delete -->
                                        <button type="button" onclick="deleteDataPengabdian('{{ $data->id }}')"
                                            class="relative group flex items-center justify-center w-10 h-10">
                                            <svg class="w-5 h-5 text-[#EF4444] group-hover:text-[#DC2626] transition"
                                                fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-lg">
                ...
                <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b gap-4">
                    <button type="button" onclick="addPengabdian()"
